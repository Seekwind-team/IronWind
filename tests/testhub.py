...
if not tests:
	testfiles = glob(os.path.dirname(os.path.abspath(__file__)) + "\\int-test*")
	for file in testfiles:
		tests.append(re.search("int-test_.*\.py", file).group(0)[:-3])


if not logfile_name:
	logfile_name = os.path.dirname(os.path.abspath(__file__)) + "\\logs\\testlog-" + datetime.now().strftime("%Y-%M-%d_%H-%M-%S") +".txt"
